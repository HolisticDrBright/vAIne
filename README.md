...
8. Deterministic, budget-aware AM/PM routines drawn from the reviewed product list imported from the Longevity Skincare AI Product Database (`data/product-database/`, `docs/consumer-catalog.md`); fictional samples are used only if that list is ever empty. Each step names the product it matched and why, or explains why it stayed category-level
